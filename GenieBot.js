            ...
            const action = parts[0];
                ...
                if (action === 'settings') {
                    ...
                    try {
                        ...
                        if(gasBuffer == null){
                            await setAsync(`settings:gas_buffer:${username}`, JSON.stringify({ gasBuffer: 10 }));

                            gasBuffer = 10 + '%';
                        }
                        if(slippage == null){
                            await setAsync(`settings:slippage:${username}`, JSON.stringify({ slippage: 3 }));
...
function generateBuyEmojis(amount, mcap) {
    ...
    const emojisCount = Math.max(1, Math.ceil(ratio / 0.000002));
